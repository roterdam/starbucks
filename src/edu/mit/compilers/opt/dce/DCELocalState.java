...
public class DCELocalState implements State<DCELocalState> {

	private Map<MidTempDeclNode, MidMemoryNode> tempMap;
	private Map<MidMemoryNode, List<MidTempDeclNode>> mentionMap;

	public DCELocalState() {
		tempMap = new HashMap<MidTempDeclNode, MidMemoryNode>();
		mentionMap = new HashMap<MidMemoryNode, List<MidTempDeclNode>>();
	}

	public DCELocalState getInitialState() {
		return new DCELocalState();
	}

	public DCELocalState getBottomState() {
		return new DCELocalState();
	}

	public DCELocalState join(DCELocalState s) {
		return new DCELocalState();
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof DCELocalState)) {
			return false;
		}
		DCELocalState other = (DCELocalState) o;
